...
public class RemoteLightControlService {
    private static volatile RemoteLightControlService mInstance;
    ...
    private RemoteLightControlService() {
        Retrofit retrofit = new Retrofit.Builder()
                .addConverterFactory(GsonConverterFactory.create())
                .addConverterFactory(ScalarsConverterFactory.create())
                .baseUrl(BFOP_URL)
                .build();
        lightControlService = retrofit.create(LightControlService.class);
    }

    public static RemoteLightControlService getInstance() {
        if (mInstance == null) {
            synchronized (RemoteLightControlService.class) {
                if (mInstance == null) {
                    mInstance = new RemoteLightControlService();
                }
            }
        }
        return mInstance;
    ...
    public void sendControlMessage(String colorString, Callback<String> callback) {
        Call<String> call = lightControlService.changeColor(APP_ID, USER_ID
